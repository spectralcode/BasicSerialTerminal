...
{
	connect(serialPort, &QSerialPort::readyRead, this, &SerialCom::readData);
	connect(serialPort, &QSerialPort::errorOccurred, this, &SerialCom::handleError);
}
...
void SerialCom::connectToComPort(const QString &comPortName, qint32 baudRate)
{
...
void SerialCom::disconnectFromComPort()
{
...
void SerialCom::sendCommand(const QString &command)
{
	if(serialPort->isOpen()) {
